...
import { runLinkedInAgent, isProvider, type Provider } from "./providers.js";

/**
 * Mirrors agents/github-agent's and agents/whatsapp-agent's agui.ts. The
 * agent loop only takes a single prompt string, not a structured history,
 * so every prior user/assistant message is flattened into a text
 * transcript passed as a system preamble, and the most recent user message
 * becomes the prompt.
 */
...
/**
 * Runs the LinkedIn agent for one AG-UI `RunAgentInput`, streaming the
...
export async function runAgUiRequest(params: RunAgUiRequestParams): Promise<void> {
  const { input, linkedinMcpUrl, defaultProvider, res, acceptHeader } = params;
  ...
  try {
    ...
    await runLinkedInAgent(provider, {
      prompt: userPrompt,
      systemPrompt,
      linkedinMcpUrl,
