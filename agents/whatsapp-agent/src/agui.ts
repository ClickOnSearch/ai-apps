...
import { runAssistantAgent, isProvider, type Provider } from "./providers.js";

/**
 * Mirrors agents/github-agent/src/agui.ts. The agent loop only takes a
 * single prompt string, not a structured history, so every prior
 * user/assistant message is flattened into a text transcript passed as a
 * system preamble, and the most recent user message becomes the prompt.
 */
...
/**
 * Runs the personal assistant for one AG-UI `RunAgentInput`, streaming the
...
export async function runAgUiRequest(params: RunAgUiRequestParams): Promise<void> {
  const { input, mcpServerUrl, defaultProvider, res, acceptHeader } = params;
  ...
  try {
    ...
    await runAssistantAgent(provider, {
      prompt: userPrompt,
      systemPrompt,
      mcpServerUrl,
